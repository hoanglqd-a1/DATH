...
transforms = transform.Compose([
    transform.RandomJitter(brightness=0.5, saturation=0.2, contrast=0.2, hue=0.1),
    transform.RandomPerspective(distortion_scale=0.5, p=0.5),
    transform.RandomAffine(degrees = 45, translate=(0.2, 0.2), scale=(0.5, 1.2)),
    transform.HorizontalFlip(p=0.5),
    transform.VerticalFlip(p=0.3),
])
...
def main():
    ...
    train_img_dir = './dataset/train/train_frames/image'
    train_msk_dir = './dataset/train/train_masks/image'
    test_img_dir  = './dataset/train/val_frames/image'
    test_msk_dir  = './dataset/train/val_masks/image'

    train_dataset = SegmentationImageDataset(train_img_dir, train_msk_dir)
    validation_dataset = SegmentationImageDataset(test_img_dir, test_msk_dir)
